...
class ShowGeneralRequest extends StatefulWidget {
  const ShowGeneralRequest({Key? key}) : super(key: key);

  @override
  _ShowGeneralRequestState createState() => _ShowGeneralRequestState();
}

class _ShowGeneralRequestState extends State<ShowGeneralRequest> {
  String? typeUser;
  String request = '';
  File? file;
  double? lat, lng;
    ...
    print(
        ...
        if (file == null) {
          ...
          await Dio().post(apiSaveRequest, data: data).then((value) {
            request = '/alro_sb/Request/$nameRequest';
            processInsertMySQL(
              name: name,
              rem: rem,
              phone: phone,
